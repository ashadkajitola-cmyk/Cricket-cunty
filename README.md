    ...
    <div id="dashboardSection">
        ...
        <div id="tabWallet" class="tab-content hidden">
            <div class="card" style="background: #e0f2f1;">
                ...
                <p style="font-size: 13px; text-align: center; color: #444; margin-top: 0;">Wallet Code: <strong id="walletCodeDisplay" style="background: #fff; padding: 2px 6px; border-radius: 4px; border: 1px dashed #00796b; color: #00796b;">--------</strong></p>
            ...
            <div class="card" style="background: #fff8e1; border: 1px solid #ffe082;">
                ...
                <p style="margin: 8px 0 0 0; font-size: 13px; color: #d32f2f; font-weight: bold; text-align: center;">Point purchase karne ke liye sampark karein.</p>
            </div>
...
<script>
    ...
    function initUserSession() {
        ...
        db.ref('users/' + myUserId).once('value', (snapshot) => {
            if(!snapshot.exists()) {
                let walletCode = generateUniqueWalletCode();
                ...
                db.ref('wallet_codes/' + walletCode).set(myUserId);
            } else {
                let userData = snapshot.val();
                if(!userData.walletCode) {
                    let walletCode = generateUniqueWalletCode();
                    db.ref('users/' + myUserId + '/walletCode').set(walletCode);
                    db.ref('wallet_codes/' + walletCode).set(myUserId);
                }
            }
            loadDashboard();
